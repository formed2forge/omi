...

{
  ...
  echo 'OMI_APPLE_DEVELOPMENT_TEAM="9536L8KLMP"'
  extract_function generate_device_suffix
  extract_function personal_ios_identifier_suffix
  extract_function ios_dev_bundle_id_for_team
  extract_function ios_dev_app_group_for_team
  extract_function ios_dev_widget_bundle_id_for_team
  extract_function held_apple_development_team_ids
...
rc=0
APPLE_DEVELOPMENT_TEAM=invalid bash -c "source '$HARNESS'; detect_apple_team_id" >/dev/null 2>&1 || rc=$?
if [[ "$rc" -ne 0 ]]; then
  pass "rejects an invalid APPLE_DEVELOPMENT_TEAM override"
else
  fail "accepted an invalid APPLE_DEVELOPMENT_TEAM override"
fi

echo "iOS development identifiers:"
got="$(HOSTNAME=ignored bash -c "source '$HARNESS'; ios_dev_bundle_id_for_team 9536L8KLMP")"
if [[ "$got" == "com.friend-app-with-wearable.ios12.development" ]]; then
  pass "keeps the registered Omi development bundle for the Omi team"
else
  fail "expected the Omi development bundle, got '${got}'"
fi

got="$(bash -c "source '$HARNESS'; generate_device_suffix() { echo testmac; }; ios_dev_bundle_id_for_team PERSONAL01")"
if [[ "$got" == "com.friend-app-with-wearable.ios12-personal01-testmac" ]]; then
  pass "uses a team-and-machine-scoped bundle for an explicit personal team"
else
  fail "expected a team-and-machine-scoped personal bundle, got '${got}'"
fi

got="$(bash -c "source '$HARNESS'; generate_device_suffix() { echo testmac; }; ios_dev_app_group_for_team PERSONAL01")"
if [[ "$got" == "group.com.friend-app-with-wearable.ios12-personal01-testmac" ]]; then
  pass "uses a team-and-machine-scoped app group for an explicit personal team"
else
  fail "expected a team-and-machine-scoped personal app group, got '${got}'"
fi

got="$(bash -c "source '$HARNESS'; ios_dev_widget_bundle_id_for_team 9536L8KLMP")"
if [[ "$got" == "com.friend-app-with-wearable.ios12.development.widget" ]]; then
  pass "keeps the registered Omi development widget bundle for the Omi team"
else
  fail "expected the Omi development widget bundle, got '${got}'"
fi

got="$(bash -c "source '$HARNESS'; generate_device_suffix() { echo testmac; }; ios_dev_widget_bundle_id_for_team PERSONAL01")"
if [[ "$got" == "com.friend-app-with-wearable.ios12-personal01-testmac.development.widget" ]]; then
  pass "uses a team-and-machine-scoped widget bundle for an explicit personal team"
else
  fail "expected a team-and-machine-scoped personal widget bundle, got '${got}'"
fi
...
make_cert() {
  local dir="$1" cn="$2" team_id="${3:-OTHERTEAM1}"
  openssl req -x509 -newkey rsa:2048 -keyout "$dir/k.pem" -out "$dir/c.pem" \
    -days 1 -nodes -subj "/CN=$cn/OU=$team_id" >/dev/null 2>&1
...
if [ "$1" = "find-certificate" ]; then cat "$FIXTURE_ROOT/certificates.txt" 2>/dev/null; exit 0; fi
exit 1
STUB
...
read -r der fp <<<"$(make_cert "$root" "Apple Development: Dev (PERSONALTM)" PAIDTEAM01)"
...
read -r der fp <<<"$(make_cert "$root" "Developer ID Application: Dev (PAIDTEAM01)" PAIDTEAM01)"
make_profile_fixture "$root" "$der" \
...
# 4. A stale profile certificate must not hide a team for which this Mac holds
#    a newer Apple Development identity. Certificate OU, not the display-name
#    suffix or profile fingerprint age, identifies the signing team.
root=$(mktemp -d -t omi_sig_d_XXXXXX)
mkdir -p "$root/current" "$root/stale"
read -r current_der current_fp <<<"$(make_cert "$root/current" "Apple Development: Dev (DISPLAYONLY)" PERSONAL01)"
read -r stale_der stale_fp <<<"$(make_cert "$root/stale" "Apple Development: Dev (OLDPROFILE)" PERSONAL01)"
make_profile_fixture "$root" "$stale_der" \
"  1) $current_fp \"Apple Development: Dev (DISPLAYONLY)\"
     1 valid identities found" \
  PERSONAL01
{
  echo "SHA-1 hash: $current_fp"
  cat "$root/current/c.pem"
} > "$root/certificates.txt"
got=$(detect_with_fixture "$root")
rm -rf "$root"
if [[ "$got" == "PERSONAL01" ]]; then
  pass "keeps a team selectable when its profile embeds an older certificate"
else
  fail "expected PERSONAL01 from the held certificate OU, got '${got}'"
fi

# 5. A profile matching this machine's generated bundle must not silently win
#    when another usable team exists. Without a TTY the function must stop and
#    request an explicit override; interactively this same branch shows the menu.
root=$(mktemp -d -t omi_sig_d_XXXXXX)
read -r der fp <<<"$(make_cert "$root" "Apple Development: Dev (PERSONALTM)")"
make_profile_fixture "$root" "$der" \
"  1) $fp \"Apple Development: Dev (PERSONALTM)\"
     1 valid identities found" \
  9536L8KLMP PERSONAL01
rc=0
output=$(FIXTURE_ROOT="$root" PATH="$root/bin:$PATH" HOME="$root/home" \
  env -u APPLE_DEVELOPMENT_TEAM bash -c "source '$HARNESS'; detect_apple_team_id < /dev/null" 2>&1) || rc=$?
rm -rf "$root"
if [[ "$rc" -ne 0 && "$output" == *"Multiple Apple Developer accounts found"* ]]; then
  pass "requires a choice when both Omi and personal teams are usable"
else
  fail "expected the multi-team choice path (rc=$rc), got '${output}'"
fi
fi
