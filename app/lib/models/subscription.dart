...
class PlanType {
  ...
  static const PlanType max = PlanType._('max', 'max', 'max');
  ...
  static const List<PlanType> values = <PlanType>[basic, unlimited, architect, operator, plus, max, unlimitedV2];
  ...
  bool get hasUnlimitedTranscription =>
      ...
      _canonicalWireName == PlanType.max.wireName ||
      _canonicalWireName == PlanType.unlimitedV2.wireName;

  /// Mirrors the Stage 0 catalog's full-desktop entitlement profiles.
  ...
  ///
  /// Core is an always-available floor rather than a purchasable SKU. Neo is
  /// current-subscriber-only, so it is not a mobile purchase target either.
