...
Live Stripe webhook persist (test-mode only, not hermetic CI): `python backend/scripts/exercise_stripe_webhooks_backend.py --apply`. That starts `stripe listen`, boots uvicorn with the listen `STRIPE_WEBHOOK_SECRET` **before import** (`utils.stripe.parse_event` reads it at module import), completes a subscription Checkout for the never-subscribed harness user `pricing_basic` (`client_reference_id`), then creates an ephemeral Plus subscription with `metadata.uid` and asserts Plus then Basic. Requires test-mode `STRIPE_API_KEY` and `STRIPE_PLUS_MONTHLY_PRICE_ID`. Never writes `recognized_stripe_prices`. The CLI signature-only pass is `exercise_stripe_webhooks.py --apply` (stdlib receiver, no Firestore). Phase 3 subscription changes: `python backend/scripts/exercise_stripe_subscription_changes.py --apply` (Test Clock when `billing_clock_write` is granted; cancel-at-period-end → Basic calls `TestClock.advance`).
