...
``customer.subscription.created|deleted`` persist plan changes in the Firestore
emulator. Never touches live/prod and never writes ``recognized_stripe_prices``.
...
def _print_dry_run(cli: Optional[str], port: int) -> None:
    ...
    print(f"  events: {', '.join(SUBSCRIBE_EVENTS)}")
    ...
    print("Creates an ephemeral Plus subscription with metadata.uid, asserts emulator plan plus then basic.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(
        f"payment.py persisted {result['plan_after_create']} then {result['plan_after_cancel']} "
