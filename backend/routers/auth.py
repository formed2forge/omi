...
async def local_dev_custom_token(
    ...
    if not normalized_uid or len(normalized_uid) > 128:
        raise HTTPException(status_code=400, detail="uid must contain 1–128 characters")
    ...
    custom_token = await run_blocking(
        critical_executor, lambda: firebase_admin.auth.create_custom_token(normalized_uid)
    )
    token = custom_token.decode('utf-8') if isinstance(custom_token, bytes) else cast(str, custom_token)

    logger.info("local-dev auth: minted emulator custom token")
