...
On **native Wayland**, the main chat window stacks above the bar (no always-on-top). Summon hides a visible main window to the tray first — same as clicking close — so the pill is not covered. That tuck only runs when Plasma supplied an `XDG_ACTIVATION_TOKEN` (otherwise hiding the main window while the bar cannot be raised leaves nothing visible).

**Plasma Command/Script and empty `XDG_ACTIVATION_TOKEN`:** if your shortcut log shows `<empty>`, Plasma is not granting raise permission. Summon may still show the bar when no other window covers the screen. With another app maximized on top, you get a taskbar flash only. Workarounds: **System Settings → Window Management → Focus → Focus Stealing Prevention → Low**, or verify the shortcut command ends with `--omi-action summon` (a bare AppImage launch surfaces the main chat window instead of the bar).
