...
export type AppSettings = {
  ...
  /** Local dev toggle for the "Core" free-tier design: when true, raw captured
   *  audio is ALSO written to a local WAV file per listen session
   *  (main/localAudio/), independent of and in addition to the existing
   *  cloud-STT streaming in ipc/omiListen.ts. Firestore/transcript sync is
   *  unaffected either way — this only controls where the raw audio BYTES are
   *  durably stored. Default OFF (opt-IN, === true): no plan/tier catalog wires
   *  this yet, so it must stay inert until something explicitly turns it on.
   *  Retention policy: main/localAudio/localAudioConfig.ts. */
  localAudioPersistenceEnabled: boolean
}
...
export function sanitizeAppSettings(raw: Partial<AppSettings> | null | undefined): AppSettings {
  ...
  return {
    ...
    // Opt-IN (=== true): default OFF. Dev toggle only — see the field doc above.
    localAudioPersistenceEnabled: r.localAudioPersistenceEnabled === true
  }
}
