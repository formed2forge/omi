...
export type AppSettings = {
  ...
  /** Prototype (Windows on-device pipeline): whether the on-device local ASR
   *  path may run at all. Default OFF — this only ever ADDS a parallel local
   *  transcription session alongside the existing cloud STT path (never
   *  replaces it; see main/localAsr/). Distinct from the dev-only
   *  OMI_LOCAL_ASR env var (preload/index.ts's capability flag): either one
   *  enables it (see renderer lib/omiListenClient.ts) — the env var remains
   *  for quick dev testing without touching Settings.
   *  EXTENSION POINT (plan/tier gating): Core/Plus/Max plan gating for
   *  on-device transcription is not implemented yet — see
   *  formed2forge/handoffs/omi-pricing.md §24 for the not-yet-landed
   *  Core/Plus/Max catalog this will eventually plug into. */
  localAsrEnabled: boolean
  /** Prototype (Windows on-device pipeline): whether local ASR's finished
   *  transcript is additionally run through the on-device "basic processing"
   *  summarizer (main/inference/localLlmService.ts) once a session stops.
   *  Default OFF. Requires the local-LLM model to already be downloaded
   *  (main/inference/modelDownloader.ts) — this toggle does not itself
   *  trigger a download; until then summarization fails non-fatally (see
   *  main/localAsr/localAsrSession.ts's stop()). Same tier-gating extension
   *  point as localAsrEnabled above. */
...
export function sanitizeAppSettings(raw: Partial<AppSettings> | null | undefined): AppSettings {
  ...
  return {
    ...
    // Both opt-IN (=== true): OFF by default, only an explicit true enables
    // them — matches the local-audio-persistence toggle's convention for
    // this same "on-device prototype capability" family of settings.
    localAsrEnabled: r.localAsrEnabled === true,
    localLlmSummaryEnabled: r.localLlmSummaryEnabled === true
  }
}
