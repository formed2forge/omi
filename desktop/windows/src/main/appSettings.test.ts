...
describe('appSettings', () => {
  ...
  it('localAsrEnabled/localLlmSummaryEnabled default OFF and round-trip an explicit opt-in', () => {
    expect(getAppSettings().localAsrEnabled).toBe(false)
    expect(getAppSettings().localLlmSummaryEnabled).toBe(false)
    setAppSettings({ localAsrEnabled: true, localLlmSummaryEnabled: true })
    _resetForTests()
    expect(getAppSettings().localAsrEnabled).toBe(true)
    expect(getAppSettings().localLlmSummaryEnabled).toBe(true)
    // Opt-IN (=== true): absent/junk stays OFF, never coerced to ON.
    expect(sanitizeAppSettings(null).localAsrEnabled).toBe(false)
    expect(sanitizeAppSettings({ localAsrEnabled: 'yes' } as never).localAsrEnabled).toBe(false)
    expect(sanitizeAppSettings({ localLlmSummaryEnabled: 1 } as never).localLlmSummaryEnabled).toBe(
      false
    )
  })
