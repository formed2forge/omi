...
describe('appSettings', () => {
  ...
  it('round-trips the local-audio persistence dev toggle (default off)', () => {
    expect(getAppSettings().localAudioPersistenceEnabled).toBe(false)
    setAppSettings({ localAudioPersistenceEnabled: true })
    _resetForTests()
    expect(getAppSettings().localAudioPersistenceEnabled).toBe(true)
  })
  ...
  it('sanitizes bad input back to safe defaults', () => {
    expect(sanitizeAppSettings({} as never)).toEqual({
      ...
      localAudioPersistenceEnabled: false
    })
    ...
    // Local-audio persistence (Core free-tier dev toggle) is opt-IN (=== true):
    // OFF by default/junk, only an explicit true turns local recording on.
    expect(
      sanitizeAppSettings({ localAudioPersistenceEnabled: true }).localAudioPersistenceEnabled
    ).toBe(true)
    expect(
      sanitizeAppSettings({ localAudioPersistenceEnabled: 'yes' } as never)
        .localAudioPersistenceEnabled
    ).toBe(false)
