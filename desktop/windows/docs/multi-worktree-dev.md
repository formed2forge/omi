...
- **Floating bar on native Wayland (niri, etc.)** — with `OMI_OZONE=wayland`,
  ...
  `setAlwaysOnTop`, and the focus-halo glow window is not created. Summon
  still needs a working global shortcut (broken on native Wayland) or an
